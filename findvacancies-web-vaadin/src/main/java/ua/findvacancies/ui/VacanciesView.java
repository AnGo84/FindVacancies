...
public class VacanciesView extends HorizontalLayout {
	...
	private final PaginatedGrid<Vacancy, String> vacanciesListGrid;
	...
	public VacanciesView() {
		...
		barAndGridVLayout.setSpacing(false);
	...
	private PaginatedGrid<Vacancy, String> initVacanciesListGrid() {
		// Create a grid bound to the list
		PaginatedGrid<Vacancy, String> grid = new PaginatedGrid<>();

		grid.setSizeFull();
		/* Show row number in grid but not item index in list */
        /*grid.addColumn(LitRenderer.of("${index + 1}"))
                .setHeader("#").setResizable(false).setWidth("30px");*/
		/**/
		...
		grid.addComponentColumn(vacancy -> new Anchor(vacancy.getUrl(), vacancy.getSiteName()))
				.setSortable(true)
				.setComparator(Comparator.comparing(Vacancy::getUrl))
		...
		grid.addColumn(
				...
				.setSortable(true)
				.setComparator(Comparator.comparing(Vacancy::getDate))
				.setId("columnDateId");
